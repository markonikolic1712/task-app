...
public class TasksAppApplication implements CommandLineRunner {
	...
	public void run(String... args) throws Exception {
		...
		for(User user : users) {
			System.out.println("UserId: " + userRepository.save(user).getId());
		}

		Task[] tasks = new Task[] {
				new Task(null,"Task 1", "Task 1 description", LocalDate.now(), TaskStatus.OPEN, TaskPriority.HIGH, Instant.now(), Instant.now(), users[0]),
				new Task(null,"Task 2", "Task 2 description", LocalDate.parse("2026-03-14"), TaskStatus.COMPLETED, TaskPriority.MEDIUM, Instant.now(), Instant.now(), users[0]),
				new Task(null,"Task 3", "Task 3 description", LocalDate.parse("2026-06-17"), TaskStatus.COMPLETED, TaskPriority.LOW, Instant.now(), Instant.now(), users[0]),
